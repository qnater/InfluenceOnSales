...
class AnalyticsGraph:
    ...
    def correlate_popularity_and_sales(self, products_popularity_scores, products_salesranks):
        ...
        print(f"\ncorrelation (& p-value) overall: {overall_corr, overall_p_value}")
        print(f"\npopularities & salesranks of the 15 best:")
        for i in range(len(best_asins)):
            print(f"\tasin: {best_asins[i]}")
            print(f"\t\t score = {sorted_popularities[i]}")
            print(f"\t\t salesrank = {sorted_salesrank[i]}")
        print(f"correlation (& p-value) of the 15 best: {best_corr, best_p_value}")
